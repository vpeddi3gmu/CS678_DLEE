...
from .wiki_data_prep import WikiDataSet, wiki_collate
from .common_utils import load_ontology, check_pronoun, clean_mention
...
class WIKI_Data_Loaders(nn.Module):
    '''
    ...
    def wiki_gold_data_gen(self, ex, ontology_dict, mark_trigger=True, index=0, ent2info=None, use_info=False):
        ...
        if len(context_words) > MAX_CONTEXT_LENGTH:
            ...
            if cur_len > MAX_CONTEXT_LENGTH:
                ...
                end_idx = min(len(context_words),
                              trigger_start + MAX_CONTEXT_LENGTH // 2)
                ...
                while cur_len > prev_len:
                    ...
                    if left >= 0:
                        left_sent_tokens = [tup[0]
                                            for tup in ex['sentences'][left][0]]
                    ...
                    if right < total_sents:
                        right_sent_tokens = [tup[0]
                                             for tup in ex['sentences'][right][0]]
                ...
                offset = sum([len(ex['sentences'][idx][0])
        ...
        if mark_trigger:
            prefix = self.tokenizer.tokenize(
                ' '.join(context_words[:trigger['start']]), add_prefix_space=True)
            tgt = self.tokenizer.tokenize(' '.join(
                context_words[trigger['start']: trigger['end']]), add_prefix_space=True)

            suffix = self.tokenizer.tokenize(
                ' '.join(context_words[trigger['end']:]), add_prefix_space=True)
            context = prefix + [' <tgr>', ] + tgt + [' <tgr>', ] + suffix
        else:
            context = self.tokenizer.tokenize(
                ' '.join(context_words), add_prefix_space=True)
        ...
        for w in space_tokenized_template:
            tokenized_template.extend(
                self.tokenizer.tokenize(w, add_prefix_space=True))

        # calculate sbert embedding
        context_emb = self.sim_model.encode(
            ' '.join(context_words), show_progress_bar=False)
        out_template_emb = self.sim_model.encode(
        ...
        if not os.path.exists(data_dir):
            ...
            for split, f in [('train', self.hparams.train_file), ('val', self.hparams.val_file), ('test', self.hparams.test_file)]:
                ...
                if self.hparams.adv and split == 'test':
                    coref_reader = open(os.path.join(
                        self.hparams.coref_dir, '{}_adv.jsonlines'.format(coref_split)))
                else:
                    coref_reader = open(os.path.join(
                        self.hparams.coref_dir, '{}.jsonlines'.format(coref_split)))
                with open(f, 'r') as reader,  open(os.path.join(data_dir, '{}.jsonl'.format(split)), 'w') as writer:
                    for line, coref_line in zip(reader, coref_reader):
                        ...
                        if ex['doc_id'] != corefs['doc_key']:
                            print(split, ex['doc_id'], corefs['doc_key'])
                        ...
                        for i in range(len(ex['event_mentions'])):
                            ...
                            if len(all_out_template_embs) > 0:
                                cosine_scores = util.pytorch_cos_sim(
                                    [context_emb], all_out_template_embs)
                                most_sim_idx = torch.argmax(
                                    cosine_scores, dim=-1)
                            ...
                            max_tokens = max(
                                len(context) + len(input_template) + 2, max_tokens)
